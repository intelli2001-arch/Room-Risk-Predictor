...
2. **리드타임 기반 예측**: 현재 시점에서 예약 시간까지의 시간차를 피처로 활용
   - 리드타임이 짧을수록 (예약이 임박할수록) 마감 확률이 높게 예측
   - 6가지 리드타임 범위로 학습 데이터 생성 (당일~1달 전)
3. **2026년 일정 반영**: 휴일, 시험기간, 공연시즌 자동 판단
4. **날짜 선택**: 달력 위젯으로 2026년 예약 희망 날짜 선택
5. **복수 시간대 선택**: 여러 시간대를 클릭하여 동시에 예약 가능
   - 클릭으로 선택/해제 토글
   - 선택된 시간대는 ✅ 표시
   - 총 이용 시간 및 금액 자동 계산
6. **위험도 예측**: ML 기반 시간대별 예약 마감 확률을 4단계로 분류
   - 여유 (0~25%) - 🟢 초록색: 지금 예약하지 않아도 충분히 여유 있음
   - 주의 (25~50%) - 🟡 노랑색: 조금씩 찰 가능성 있음
   - 임박 (50~75%) - 🟠 주황색: 예약 지연 시 확보 어려움
   - 위험 (75~100%) - 🔴 빨간색: 빠른 예약 필요
6. **과거 날짜/시간 차단**: 이미 지난 날짜와 시간대는 예약 불가
7. **기간 특성 표시**: 휴일/시험기간/공연시즌 배지 표시
8. **예약 시점 표시**: "🕐 X일 전 (리드타임: Y시간)" 형식으로 표시
9. **시각화**: Plotly 막대그래프로 시간대별 위험도 표시
10. **예약하기**: PoC 결제 화면 안내
