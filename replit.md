# 연습실 예약 마감 위험도 예측 PoC (2026)

## Overview
연습실 예약 플랫폼의 "시간대별 예약 마감 위험도 예측" 기능을 시각적으로 보여주는 PoC(Proof of Concept) 웹 앱입니다.
**머신러닝(RandomForest) 기반** 예측 모델을 사용하며, **고객**과 **사업자** 모두를 위한 기능을 제공합니다.

## Tech Stack
- Python 3.11
- Streamlit (UI)
- pandas, numpy (데이터 처리)
- scikit-learn (RandomForest ML 모델)
- plotly (시각화)

## Rooms & Pricing
- **연습실 종류**:
  - A룸 (소형): 1~3인용
  - B룸 (중형): 3~5인용
  - C룸 (대형): 5~8인용
- **기본 시간당 요금**: 6,000원
- **오픈연습실 (🎸)**: 같은 날 5,000원 정액 (시간 관계없이)
- **타임세일 (💰)**: 30% 할인 적용

## Features

### 👤 고객용 기능
1. **ML 모델 학습**: RandomForest 모델로 예약 패턴 학습
2. **리드타임 기반 예측**: 현재 시점에서 예약 시간까지의 시간차를 피처로 활용
   - 리드타임이 짧을수록 (예약이 임박할수록) 마감 확률이 높게 예측
   - 6가지 리드타임 범위로 학습 데이터 생성 (당일~1달 전)
3. **2026년 일정 반영**: 휴일, 시험기간, 공연시즌 자동 판단
4. **날짜 선택**: 달력 위젯으로 2026년 예약 희망 날짜 선택
5. **복수 시간대 선택**: 여러 시간대를 클릭하여 동시에 예약 가능
   - 클릭으로 선택/해제 토글
   - 선택된 시간대는 ✅ 표시
   - 총 이용 시간 및 금액 자동 계산
6. **위험도 예측**: ML 기반 시간대별 예약 마감 확률을 4단계로 분류
   - 여유 (0~25%) - 🟢 초록색: 지금 예약하지 않아도 충분히 여유 있음
   - 주의 (25~50%) - 🟡 노랑색: 조금씩 찰 가능성 있음
   - 임박 (50~75%) - 🟠 주황색: 예약 지연 시 확보 어려움
   - 위험 (75~100%) - 🔴 빨간색: 빠른 예약 필요
7. **이미 마감된 시간대**: 수요 기반으로 일부 시간대가 이미 마감됨 (🚫 표시)
8. **과거 날짜/시간 차단**: 이미 지난 날짜와 시간대는 예약 불가
9. **기간 특성 표시**: 휴일/시험기간/공연시즌 배지 표시
10. **예약 시점 표시**: "🕐 X일 전 (리드타임: Y시간)" 형식으로 표시
11. **시각화**: Plotly 막대그래프로 시간대별 위험도 표시
12. **예약하기**: PoC 결제 화면 안내

### 🏢 사업자용 기능
1. **공간 활용률 분석**: ML 예측 데이터를 기반으로 수요 패턴 시각화
   - 월별 평균 수요
   - 요일별 평균 수요
   - 시간대별 평균 수요
   - 기간 특성별 수요 (평일/휴일/시험기간/공연시즌)
2. **프로모션 관리**: 저수요 시간대를 특별 운영 모드로 전환
   - 🎸 오픈연습실: 누구나 자유롭게 이용 가능한 개방 시간
   - 💰 타임세일: 할인된 가격으로 예약 유도
   - 저수요 기준 임계값 설정 가능 (10~50%)
   - 날짜별 시간대별 프로모션 상태 관리

## 2026년 주요 일정
- **휴일**: 1/1, 3/1, 3/2, 5/5, 5/24, 6/6, 8/15, 10/3, 10/9, 12/25
- **시험기간**: 4/13~4/24, 6/8~6/19, 10/12~10/23, 12/7~12/18
- **공연시즌**: 5/11~6/5, 11/2~11/27

## Project Structure
```
├── app.py                    # 메인 Streamlit 앱 (Python 백엔드)
├── index.html                # 정적 HTML 앱 (Netlify 배포용)
├── netlify.toml              # Netlify 배포 설정
├── attached_assets/          # 첨부 파일 (CSV 데이터, 이미지)
├── .streamlit/config.toml    # Streamlit 설정
└── pyproject.toml            # Python 패키지 의존성
```

## Running the App

### Streamlit 버전 (Replit)
```bash
streamlit run app.py --server.port 5000
```

### 정적 HTML 버전 (Netlify)
index.html 파일을 Netlify에 배포하면 됩니다.
- Netlify에서 GitHub 연동 또는 폴더 드래그 드롭으로 배포
- netlify.toml 설정 파일 포함됨

## Data Format
CSV 데이터 컬럼:
- 연도, 월, 일, 요일 (0=일요일~6=토요일)
- 시간 (9~22시)
- 휴일 여부, 시험기간 여부, 공연시즌 여부
- 예약 여부 (0 또는 1)
- 리드타임_시간, 취소 여부

## Notes
- PoC 데모이므로 실제 결제 연동 없음
- RandomForest ML 모델 기반 예측 (scikit-learn 사용)
- SpaceCloud (https://www.spacecloud.kr) 참고 디자인
